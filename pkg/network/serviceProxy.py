...
class ServiceProxy:
    ...
    def __init__(self):
        ...
        self.iptables_available = (
            not self.is_macos and shutil.which("iptables") is not None
        )
    ...
    def setup_base_chains(self):
        ...
        try:
            ...
            self._run_iptables(
                ["-t", "nat", "-I", "OUTPUT", "-j", self.nat_chain], ignore_errors=True
            )
    ...
    def create_service_rules(
        ...
        if self.is_macos or not self.iptables_available:
            self.logger.info(
                f"模拟创建Service {service_name}的iptables规则 (ClusterIP: {cluster_ip}:{port})"
            )
            return
    ...
    def _run_iptables(self, args: List[str], ignore_errors: bool = False):
        ...
        try:
            ...
            if not ignore_errors:
                self.logger.error(
                    f"iptables命令执行失败: {' '.join(cmd)}, 错误: {e.stderr}"
                )
                raise
            else:
                self.logger.debug(
                    f"iptables命令执行失败(忽略): {' '.join(cmd)}, 错误: {e.stderr}"
                )
            ...
            for line in result.stdout.split("\n"):
                if self.endpoint_chain_prefix in line:
                    ...
                    if chain_name and chain_name.startswith(self.endpoint_chain_prefix):
                        self._run_iptables(
                            ["-t", "nat", "-F", chain_name], ignore_errors=True
                        )
                        self._run_iptables(
                            ["-t", "nat", "-X", chain_name], ignore_errors=True
                        )
