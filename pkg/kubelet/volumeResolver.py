...
class VolumeResolver:
    ...
    def resolve_volumes(
        self, pod_volumes: List[Dict], namespace: str
    ) -> Dict[str, str]:
        """
        解析Pod卷配置到实际挂载路径
        只支持PVC类型的卷
            ...
            if volume_type == "pvc":
                ...
                if pvc_name:
                    ...
                    if mount_path:
                        ...
                        print(
                            f"[ERROR] Failed to resolve PVC {pvc_name} in namespace {namespace}"
                        )
                else:
                ...
                # 不支持的卷类型
                print(
                    f"[WARN] Unsupported volume type '{volume_type}' for volume '{volume_name}', only PVC is supported"
                )
    ...
    def _resolve_pvc(self, pvc_name: str, namespace: str) -> Optional[str]:
        ...
        try:
            # 获取PVC信息
            pvc_url = self.uri_config.PVC_SPEC_URL.format(
            ...
            if isinstance(pvc_response, str):
                print(
                    f"[ERROR] API returned string response for PVC {pvc_name}: {pvc_response}"
                )
                return None
            ...
            if not isinstance(pvc_data, dict):
                print(
                    f"[ERROR] Expected dict for PVC data, got {type(pvc_data)}: {pvc_data}"
                )
                return None
            ...
            if isinstance(pv_response, str):
                print(
                    f"[ERROR] API returned string response for PV {bound_pv_name}: {pv_response}"
                )
                return None
            ...
            if not isinstance(pv_data, dict):
                print(
                    f"[ERROR] Expected dict for PV data, got {type(pv_data)}: {pv_data}"
                )
                return None
        ...
        try:
            ...
            if platform.system() == "Linux":
                ...
                if result.returncode == 0:
                    self.mounted_nfs_volumes[mount_point] = {
                        "server": server,
                        "path": path,
                        "type": "native",
                    }
                    print(
                        f"[INFO] Successfully mounted NFS volume: {server}:{path} to {mount_point}"
                    )
                    mount_success = True
                else:
            ...
            if not mount_success:
                ...
                if self._setup_ssh_nfs_mirror(server, path, mount_point):
                    ...
                    print(
                        f"[INFO] Successfully set up SSH mirror for NFS volume: {server}:{path} to {mount_point}"
                    )
                    mount_success = True
    ...
    def _setup_ssh_nfs_mirror(
        self, server: str, remote_path: str, local_path: str
    ) -> bool:
        """
        通过SSH创建NFS的本地镜像目录
        ...
        try:
            ...
            print(
                f"[INFO] Note: This is a local mirror, data persistence depends on SSH sync"
            )
    ...
    def get_container_volume_mounts(
        self, volume_mounts: List[Dict], resolved_volumes: Dict[str, str]
    ) -> List[str]:
        """
            ...
            if volume_name in resolved_volumes:
                ...
                if not os.path.exists(host_path):
                    try:
                        ...
                        print(
                            f"[WARN] Failed to create host directory {host_path}: {str(e)}"
                        )
            ...
            try:
                ...
                if mount_type == "native":
                    ...
                    if platform.system() in ["Linux", "Darwin"]:
                        ...
                        result = subprocess.run(
                            unmount_cmd, capture_output=True, text=True
                        )
                        if result.returncode == 0:
                            print(f"[INFO] Unmounted NFS volume at {mount_path}")
                        else:
                            print(
                                f"[WARN] Failed to unmount {mount_path}: {result.stderr}"
                            )
                ...
                try:
                    ...
                    print(
                        f"[WARN] Failed to remove mount directory {mount_path}: {str(e)}"
                    )
